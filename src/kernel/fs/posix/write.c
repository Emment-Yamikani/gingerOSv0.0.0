...
ssize_t posix_file_write(struct file *file, void *buf, size_t size)
{
	...
	if (file->flags & O_NONBLOCK) {	/* Non-blocking I/O */
		if (vfs_file_can_write(file, size)) {
			...
			ssize_t retval = vfs_write(file->inode, file->offset, size, buf);

			/* Update file offset */
			file->offset += retval;
			
			/* Wake up all sleeping readers if a `read_queue' is attached */
			if (file->inode->read_queue)
				thread_queue_wakeup(file->inode->read_queue);
		...
		while (size) {
			size -= vfs_write(file->inode, file->offset, size, buf);
			...
			thread_queue_sleep(file->inode->write_queue);
		}
